describe('TokenServiceTest', function () {
    ...
    it('check "create" method', function () {
        return promise
            .then(function () {
                return Nlic.TokenService.create(context, token);
            })
            .then(function (entity) {
                expect(entity instanceof Nlic.Token).toBe(true);
    ...
    it('check "get" method', function () {
        return promise
            .then(function (entity) {
                return Nlic.TokenService.get(context, token.getProperty('number'));
            })
            .then(function (entity) {
                expect(entity instanceof Nlic.Token).toBe(true);
    ...
    it('check "list" method', function () {
        return Nlic.TokenService.list(context)
            .then(function (entities) {
                expect(Array.isArray(entities)).toBe(true);
                expect(entities.length).toBeGreaterThanOrEqual(1);
                expect(entities[0] instanceof Nlic.Token).toBe(true);
            });
    });
    ...
    it('check "delete" method', function () {
        return promise
            .then(function () {
                return Nlic.TokenService.delete(context, token.getProperty('number'));
            })
            .then(function (state) {
